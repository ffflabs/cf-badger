...
export class Badger extends GithubIntegrationDurable implements DurableObject {
  ...
  async user({ code, installationId }: { code: string, installationId?: number }): Promise<Response> {
    const dummyUserInfo = {
      hash: this.state.BADGER_KV_ID, login: 'cf-badger',
      id: this.state.APP_ID, token: this.state.GITHUB_TOKEN
    }
    console.info({ dummyUserInfo })
    this.state.storage.put(
      this.state.BADGER_KV_ID as string, dummyUserInfo)
